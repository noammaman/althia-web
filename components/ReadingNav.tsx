...
export default function ReadingNav({ product = 'Althia' }: { product?: 'Althia' | 'Mythia' | 'Senthia' | 'Ethia' | 'Unplug League' }) {
  const root = product === 'Senthia' ? '/senthia' : product === 'Ethia' ? '/ethia' : product === 'Unplug League' ? '/unplugleague' : ''
  return <aside className={styles.rail}>
    <nav aria-label={`${product} reference pages`}>
      ...
      {product === 'Ethia' && <Link href="/ethia/support">Support</Link>}
